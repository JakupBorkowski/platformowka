...
public:
	...
	virtual int kolizjaJeden(cFizyka &X);
	//wykrywanie kolizji z innym obiektem (funkcja przekazuje 1 gdy jest kolizja 0 gdy brak)
	virtual int kolizjaDwa(cFizyka &X);
	virtual int kolizjaSkok(cFizyka &X);
	virtual int kolizjaSkok2(cFizyka &X); 
	virtual int kolizjaSkok3(cFizyka &X);
	void powrotNaStart();
